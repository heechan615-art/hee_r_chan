...
# ----------------------------- 5) 관련주 찾기 (테마 → 관련 상장사) -----------------------------
def related_stocks(keyword):
    """키워드/테마 → 한국·미국 관련 상장사 각 10개+ (특징·비즈니스 모델 포함).
    Gemini 지식 기반 추정. 실패 시 None."""
    prompt = (
        "너는 한국·미국 주식에 정통한 증권사 애널리스트야. "
        f"투자 테마/키워드 '{keyword}'와 직접 관련된 상장 기업을 찾아 정리해.\n\n"
        "아래 JSON 스키마로만 답해(설명·마크다운 없이 순수 JSON):\n"
        "{\n"
        '  "keyword": "입력 키워드",\n'
        '  "summary": "이 테마가 무엇이고 왜 주목받는지 2~3문장",\n'
        '  "kr": [ {"name":"한국 상장사명", "ticker":"6자리 종목코드", '
        '"tag":"핵심 한 줄(예: HBM 장비 1위)", '
        '"desc":"이 회사의 특징과 비즈니스 모델, 그리고 이 테마와의 실제 연관(밸류체인·수혜 논리)을 2~3문장"} ],\n'
        '  "us": [ {"name":"미국 상장사명", "ticker":"티커", "tag":"핵심 한 줄", '
        '"desc":"특징·비즈니스 모델·테마 연관 2~3문장"} ]\n'
        "}\n\n"
        "작성 규칙:\n"
        "- kr(한국)·us(미국) 각각 **최소 10개**(가능하면 12~15개). 도저히 10개가 안 나오는 "
        "니치 테마여도 **최소 5개**는 채워. 관련성 높은 순으로 정렬.\n"
        "- 반드시 실재하는 상장사만. 티커/종목코드는 아는 범위에서 정확히(모르면 빈 문자열).\n"
        "- 대장주·핵심 수혜주를 앞에, 파생·간접 수혜를 뒤에.\n"
        "- 존댓말. 정보 제공 목적이며 매수·매도 권유 아님. 반드시 유효한 JSON.")
